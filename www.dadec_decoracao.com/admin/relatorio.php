...
function gerar_relatorio_mensal($mes, $ano) {
    ...
    $sql = "SELECT cliente, tipo_pagamento, data, total, estado FROM pagamento WHERE MONTH(data) = ? AND estado = 'aprovado' AND YEAR(data) = ?";
...
function calcular_total_mensal($mes,$ano) {
    ...
    $sql = "SELECT SUM(total) AS total_mensal FROM pagamento WHERE MONTH(data) = ? AND estado = 'aprovado' AND YEAR(data) = ?";
    $stmt = $connection->prepare($sql);
...
function calcular_total_produto($produto) {
    ...
    // Prepara a consulta SQL para somar os valores da coluna "total" para o mês específico
    $sql = "SELECT SUM(total) AS total_produto FROM pagamento WHERE produto LIKE ? AND estado = 'aprovado'";
    $stmt = $connection->prepare($sql);
...
function gerar_relatorio_anual($ano) {
    ...
    $sql = "SELECT cliente,tipo_pagamento,data,total,estado FROM pagamento WHERE YEAR(data) = ? AND estado = 'aprovado'";
    $stmt = $connection->prepare($sql);
...
function gerar_relatorio_produto($produto) {
    ...
    $sql = "SELECT cliente,produto,tipo_pagamento, data, total, estado FROM pagamento WHERE produto LIKE ? AND estado = 'aprovado'";
    $stmt = $connection->prepare($sql);
...
function calcular_total_anual($mes) {
    ...
    // Prepara a consulta SQL para somar os valores da coluna "total" para o mês específico
    $sql = "SELECT SUM(total) AS total_anual FROM pagamento WHERE YEAR(data) = ? AND estado = 'aprovado'";
    $stmt = $connection->prepare($sql);
        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid p-0">
                    <h1 class="h3 mb-3">Relatórios  de materiais alugados</h1>
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
